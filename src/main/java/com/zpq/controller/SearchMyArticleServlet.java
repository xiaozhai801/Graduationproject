...
public class SearchMyArticleServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		articleinfo.setArticleId(Long.parseLong(articleIdStr)); // 文章ID
		articleinfo.setTopic(topic); // 标题关键词
		articleinfo.setName(name); // 作者名
		// 类型为“全部”时不参与搜索（置空）
		articleinfo.setTypeName(("全部机型".equals(typeName)) ? null : typeName);
		articleinfo.setRelease(Integer.parseInt(releaseStr)); // 发布状态
		...
		try {
			...
			if (myarticle.equals("true")) {
				vo = articleService.searchMyArticles(articleinfo, page, limit);
			}else if (myarticle.equals("false")) {
				vo = articleService.searchMyDraftArticles(articleinfo, page, limit);
			}
